...
*Corrects `§ P6`. Context: #138.* The script scanned the **whole** `Blocked by:` line for `#N`, so every issue and pull request named in an edge's *reason* became an edge of its own. Nothing prunes such a phantom: a satisfied edge's line stays in the body permanently as provenance (`§ A2`, `turfgps-board-ops § Satisfied is not removed`), so the story is held forever on a relationship nobody declared, and the exposure grows as prose accumulates cross-references. The script now parses only the **declared** list. Where that list ends is a property of the grammar, so it is stated in the grammar's one home — `turfgps-board-ops § The dependency representation`, per `§ P2` — and is cited here rather than restated. Measured over the 112 live `Blocked by:` lines on 2026-08-29: 49 over-extracted, 48 of them a harmless repeat of the blocker's own number that the membership test already absorbed, and one real phantom — `#136`, held on `PR #67`, merged 2026-08-22, one week earlier. **Those four figures are properties of the corpus, and they reproduce exactly**: independently re-implemented extractors returned 112 / 49 / 48 / 1 unchanged at each cycle of PR #147, the last on 2026-08-29. **How many stories those lines span is not such a figure** — it is a live-board population count that moves whenever an issue is filed, closed or edited, so it is stated with an as-of basis and in this one place: **97 stories as of 2026-08-29**. This amendment first recorded 96, and a bare number carrying no as-of basis is what let that stand unnoticed.

**These figures supersede #138's own, and the supersession is recorded here rather than left for a reader to notice.** The issue reports **9** over-extracting lines, and describes `gh issue view` on a pull request as yielding nothing (`st=""`); the measured count is **49 of 112**, and that call returns `MERGED` with exit 0. `@engineering-lead`, who filed #138, withdrew both claims on the issue itself on 2026-08-29 and asserted no replacement count. The count recorded here is the one carrying a stated, runnable method: reproduced independently by three review lanes on PR #147, and re-measured against the live board at that PR's fourth cycle — 112 lines, 49 over-extracting, 48 self-repeats, 1 genuine phantom, and **0 lines losing a declared blocker**. **That last bound is attested against the parser as it now stands and not against the one cycle 2 shipped**, the boundary having moved twice since: re-run on 2026-08-29, every line's declared list is an order-preserving prefix of the pre-fix whole-line scan of the same line, so every difference between the two is a reference dropped from behind the declared run and none is dropped from inside it. #138's conclusion is untouched by the correction: one phantom held one story permanently, and the defect is real at any of these counts.
