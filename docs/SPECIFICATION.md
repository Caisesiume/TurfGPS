...
The second case needs a stated tolerance, since the system models no capture area at all. That tolerance, and the reasoning that fixes it, are under *Direct-access tolerance* in `CalculationSpecification.md`. What it decides is which walk-safety regime a candidate must pass, not merely which cost model prices it: inside the tolerance a zone is checked for level compatibility and barriers, while beyond it the zone enters the park-and-walk path, where it must produce a walkable route and an elevation profile and may be priced with a short walk, downgraded to uncertain, or excluded on that evidence. A zone is therefore not lost by falling outside the tolerance — it is examined more strictly.

**The tolerance chooses between two branches and exempts a candidate from nothing outside them.** Until 7 August 2026 the sentence above read *level compatibility and barriers alone*, and that word was load-bearing against the clause before it: a sentence stating that the tolerance decides which validation a candidate must pass, and then naming two validations exhaustively, excludes every other check in this document from the direct branch. It excluded three — the road-class and speed-limit exclusions under *§ Enforceable exclusions*, the level-compatibility requirement in the fuller form stated under *§ Direct road-access validation*, and the requirement that there be an established stopping position at all, under *§ What establishes a stopping position*. An implementer following it built a direct-access path carrying **no road-class gate whatever**, which reaches the outcome *§ An unknown speed limit fails the check* names as the single worst one that exclusion exists to prevent. The three bind inside the tolerance exactly as they bind beyond it.
...
Three further conditions apply, each defined in full elsewhere and not restated here: the road-class and speed-limit exclusions under *Enforceable exclusions*, the level-compatibility requirement under *Direct road-access validation*, and the requirement that the position the car stops at be an established one at all, under *§ What establishes a stopping position*.

**The third joined this list on 7 August 2026**, with the section that settled what establishes a position. A reader who remembers two is remembering the list as it stood before that section existed. The count was also wrong in the other direction until that date: this line named the road-class and speed-limit exclusions as applying to the direct class while the tolerance paragraph above declared them inapplicable to it, so one subsection stated two checks and only two, then named a third.
...
* No stop may be proposed on a motorway, a motorway link, a road flagged as a motorroad, a trunk road, an expressway, or any road whose speed limit is not established to be at or below the maximum for a stopping road, defined as a constant under *The maximum speed limit for a stopping road* in `CalculationSpecification.md`. A limit that is absent, unparseable, or unit-ambiguous does not establish this and excludes the road, per *An unknown speed limit fails the check*. A nearby rest area, service road, parking area, or exit may still make the zone accessible; the high-speed carriageway itself never is.
...
#### The class limb does not rest on the speed limb

Until 7 August 2026 the exclusion named two road classes, motorway and motorway link, and those were the only classes excluded anywhere in this documentation set. Motorroad, trunk, and expressway appeared nowhere in it.

**The speed limb does not catch them, and the gap is arithmetic rather than marginal.** The limb admits a road at or below the maximum, so a road posted at exactly the maximum passes. A Swedish trunk road posted at exactly that figure therefore clears the speed limb, clears drivability, clears the level and access-path exclusions where the terrain is ordinary, is not a route through a restricted area, and is not a capture from a moving vehicle. It clears all six exclusions and is classified **confidently** — a stop proposed on a high-speed rural carriageway with every check reporting a pass. Adding the classes is what closes it; moving the constant is not, because the constant is a proposal with no origin and lowering it to exclude one country's trunk network would remove ordinary roads everywhere else.

**The classes are decided here; their tag spellings are not, and this document must not be read as supplying them.** The OSM extract of *Architecture.md § D5* does not exist yet, so nobody in this repository has checked how any of the three is spelled in it, whether one is a value and another a flag, or whether a national network uses a class this list still does not name. The spellings are owed confirmation against the extract and are deferred to the OSM-derived feature tables under *Architecture.md § Still owed by this document* — the same footing the establisher features of *§ What establishes a stopping position* already sit on, and for the same reason: those tables are a safety surface owed their own design and their own review. What is settled here is which **classes** are refused. An implementation that finds a spelling wrong corrects the spelling and never narrows the set.
...
**Only a numeric value with a determinable unit establishes the limit.** A number already in km/h, or a number carrying a unit that can be determined and converted, is compared against the maximum, and the road passes or fails on that comparison. Every other form leaves the limit unestablished, and an unestablished limit excludes the road.

**The forms below are illustrations, not an enumeration.** A tagging form absent from this list does not thereby establish the limit — it excludes the road exactly as the listed forms do. This is said explicitly because the list reads as though it were exhaustive and nothing has established that it is: these cases were found by looking, and only as many were found as were looked for.

Phrased the other way round — excluding only roads whose *recorded* limit exceeds the maximum — an unrecorded limit would fail to trigger the exclusion and the road would be admitted. That is the wrong direction, and these tagging cases show why it is not a pedantic distinction:
...
* **`maxspeed=60 mph` read as the integer 60** compares 60 against a maximum expressed in km/h and passes, when the road is in fact 96.6 km/h and must fail. **The defect is the silent coercion, not the presence of a unit.** A value whose unit is determinable is converted and *then* compared, passing or failing on the converted figure: `maxspeed=60 mph` becomes 96.6 km/h and fails, while `maxspeed=30 mph` becomes 48.3 km/h and passes — as it must, being an ordinary urban street of exactly the kind this product exists to serve. Failing every unit-bearing value outright would remove the UK *urban* network along with the rural one described under *The United Kingdom is a park-and-walk market*, which is neither intended nor stated anywhere. Only a value whose unit **cannot** be determined is unit-ambiguous, and that is what fails under the rule above. What must never happen is a unit-bearing value stripped of its unit and compared as though it were already km/h.
* **A jurisdictional, implicit, or symbolic value** — `DE:urban`, `GB:national`, `zone:30`, `walk`, `signals`, `variable` — does not establish the limit, and the road is excluded. Such a value is neither absent nor unit-ambiguous, so the tempting treatment is to resolve it against a table of national defaults. That is precisely what must not happen: resolving it would **establish a limit the map data never recorded**, which is upgrading unknown to permitted by defaulting — the failure this entire sub-section exists to forbid. The resolved figure would rest on a country attribution supplied by the resolver rather than by the data, and it would arrive as a plausible number carrying no sign that it had been inferred at all. **No lookup table sits on this safety path.**

  `GB:national` shows the ambiguity is not only jurisdictional but internal to a single code: it means 96.6 km/h on a rural single carriageway and 112.7 km/h on a dual, disambiguated only by a carriageway type that is not always recorded either. Both figures exceed the current maximum, so this particular code would exclude the road on resolution as well — but that is a coincidence of the constant's present value, not a property of the rule, and nothing may lean on it. A code whose branches straddle the maximum resolves to a pass on one branch and a fail on the other, decided by a fact the data did not carry.
* **A road carrying more than one limit** — direction-dependent (`maxspeed:forward` and `maxspeed:backward`), vehicle-class-dependent, or conditional (`maxspeed:conditional`) — is checked against the **highest applicable value**. That is the limit traffic may be travelling at in the circumstance the check cannot rule out, and a stop has to be safe in that circumstance rather than in the most favourable one. Where which value applies cannot be determined, nothing is established and the road is excluded.
...
The UK national speed limit is 60 mph — **96.6 km/h** — on a rural single carriageway, and 70 mph, or 112.7 km/h, on a dual carriageway. Both are above the maximum for a stopping road. Essentially the entire UK rural network at national speed limit is therefore unavailable as a stopping place, and Great Britain is the second-largest national share of the zone corpus after Sweden.

**Those two limits are recalled, not verified.** They are UK road law and citable in principle, but no one in this repository has checked them against a published source, and they are registered as unverified under *Estimate accuracy and calibration*. The corpus share this boundary also rests on is measured and dated; these figures are not. A reader must not take the whole boundary as measured because half of it is. The citation belongs here, once someone verifies them.
...
**This section is not a source for the maximum and must not be cited as one.** What is measured here is the *consequence* of the constant against the zone corpus, not the constant itself. The figure remains a proposed default with no origin, no measurement, and no cited source, exactly as recorded at its definition site. This guard is stated in both places deliberately: the same measurement appears here in product terms and there in numeric terms, and a reader who arrives at only one of them must not leave it believing the constant has been established by anything either says.
...
**A stopping position is established only where the map data carries a feature that is itself a place to stop** — a mapped parking area, a lay-by, a rest area, a service road, or an attribute explicitly recording that stopping is permitted. The *kinds* of feature are named here and the attributes identifying each of them deliberately are not: that detail belongs to the OSM-derived feature tables owed under *Architecture.md § Still owed by this document*, which records them as a safety surface owed their own design and their own review. This document is not their home.

**A shoulder was listed here until 7 August 2026 and is struck.** Every other establisher is a distinct mapped feature with its own geometry and its own attributes, and that is precisely what makes the independent binding below true of them: the exclusions get a second object to test, so a lay-by beside a refused road fails on the road while the lay-by is real. A shoulder has no separate object. Establishing a position on it and asking whether the road admits a stop are **the same test on the same way, run twice**, so the guard below cannot bind and the criterion cannot fail. Nothing is lost by removing it: the attributes the data plane will carry are named under *Architecture.md § D4*, and a shoulder attribute is not among them, so the criterion was unfailable against data the store was never going to hold. Lay-by, rest area, service road, and parking area already carry the mapped-safe-place-beside-a-road case this was reaching for. **It is struck on that reasoning and is not forbidden forever** — when the feature tables above are authored against a real extract, a shoulder that turns out to be a separately mapped object with its own attributes may be proposed again on that evidence.
...
**A restriction recorded on the feature is decisive against establishment, and it sends the position to uncertain.** This completes a symmetry the paragraph above states in one direction only. Absence of a restriction is never evidence of permission; **presence of one is evidence, and it is conclusive** — a mapped parking area tagged customers-only, a private access, a permit-holders-only bay, a service road recorded as access-restricted. Where the data records a restriction against the feature itself, the feature does not establish a position, and reading the restriction as a soft signal to be weighed against the feature's existence is the same defaulting failure running the other way.

**Uncertain, not excluded, and the distinction is doing real work here.** The position falls to uncertain, so the user may still place the stop there and have it recorded as user-chosen. A customers-only car park where the driver actually is a customer stays reachable, because the restriction is one the driver can satisfy and the system cannot see that they have. A gated staff car park stops being called confident. Excluding on a recorded restriction would remove both, and the system has no way to tell them apart — which is exactly the judgement *§ Handling the uncertain bucket* exists to hand back to the driver.

**The private-and-access-restricted exclusion does not reach this and must not be read as covering it.** That member of *§ Enforceable exclusions* refuses a **route constructed through** an area the data marks private or access-restricted. Its antecedent is a route passing through an area, not a stop at a position inside one, so it does not bind here, and the independent-binding rule below does not reach it either. This paragraph is what refuses the restricted stop; nothing else in this document did.

A candidate therefore resolves three ways rather than two:

* **Established** — the data carries a stopping feature at the position, the data records no restriction against that feature, and the position survives the exclusions. The zone may be classified confidently, subject to everything else this section requires of it.
* **Uncertain** — no exclusion applies, and either no stopping feature is identified there, or one is identified and the data records a restriction against it. The zone survives into review carrying no time estimate and no score. The two routes into this bucket differ in what the user is told, not in what the system does: the first reports that nothing was found, the second reports the restriction it found.
* **Excluded** — an enforceable exclusion applies. The zone is removed, whatever features sit near it.

**The exclusions bind independently, and a feature never rescues a position from one.** *§ Enforceable exclusions* applies to the position regardless of what the data carries there. A lay-by mapped on a road whose recorded limit cannot be parsed is the case to hold in mind: the feature is real, and it would establish the position on its own, and the road is refused anyway under *§ An unknown speed limit fails the check*, so no position is established there. Establishment is a necessary condition for a confident classification and never a sufficient one.

**Which exclusions reach both access branches is not uniform, and this line claimed more than the list delivers.** Until 7 August 2026 it read that they apply *on both access branches* without qualification. That is true of the two whose obligation is on the **stop** — the road-class and speed-limit exclusion, and drivability — and a park-and-walk stopping position is subject to each exactly as a direct one is. It is not true of level compatibility, whose own antecedent reads *no zone may be classified as directly road-accessible where…* and is textually one branch. No hazard follows, because the park-and-walk side is held by the access-path exclusion, which refuses a path that is absent, disconnected, or implausibly steep. What was wrong was the claim and not the coverage, and a claim wider than its list is how a reader comes to believe a check exists that nobody wrote.
...
Three things already in this document carry more weight as a result.

*§ Handling the uncertain bucket* now carries more than the rural candidates it was written for.

*§ This should be measured, not assumed* earmarks the reserve pool for removal if users do not accept what it offers, and removing it while this definition stands would make those candidates invisible rather than uncertain — the exclusion this definition refuses, arriving by the back door. **The coupling is measurement validity and not sequencing.** This definition changes the pool's composition in the direction that depresses the acceptance rate: it moves candidates in that were previously offered confidently or not at all, so a rate measured after it is not a rate about the feature that earmark was written against. Deciding the two together is therefore not enough, because both decisions would rest on the same invalid number. **The mitigation attaches to the metric** — acceptance recorded per uncertainty reason, so a thin-data candidate and a restricted-feature one are not averaged into one figure, or the earmark re-baselined against the post-ruling pool before any removal is argued from it.

And the reserve pool now has a **dead sensor** beneath it. *§ Reconciling this with the absolute ceiling* refuses an acceptance that would breach the ceiling, and the only thing it has to test an unpriced stop against is the conservative upper bound — which has no value in any document.
...
**The reserve pool does not operate at all until that bound has a configured value.** The bound is filed under *CalculationSpecification.md § Enforcement constants that do not yet exist*, with no value and no origin anywhere. Where it is unset, the system offers no reserve candidates and says so, rather than offering them against a ceiling check that cannot fire. Refusing to run is the correct posture for a safety limb with no sensor, and it is the posture *§ Enforceable exclusions* already takes elsewhere: a check that cannot be evaluated fails, and never passes by default.

**What the alternative costs is the whole argument.** Running with the bound unset does not disable the ceiling visibly — it leaves an implementation free to compute the missing figure optimistically, at which point the ceiling check passes, the refusal never fires, and the user accepts a reserve zone that commits them past the allowance they themselves stated, **with every check reporting compliance.** There is no signal anywhere in that sequence. A refusal to operate is loud, is discovered on the first deployment, and costs a feature that is itself earmarked for removal on evidence; a silent optimistic default costs the user the one constraint they set.
...
#### One external fact is asserted but not verified: the UK national speed limits

The 60 mph rural-single-carriageway and 70 mph dual-carriageway figures — used under *The United Kingdom is a park-and-walk market*, and again at the constant's definition site in `CalculationSpecification.md` — are **recalled UK road law that no one in this repository has checked against a published source.** They are citable in principle and uncited in fact. Verifying them is a lookup rather than a measurement, so they do not widen the measurement effort described above; whoever performs that lookup replaces this entry with the source and the date it was checked, per the origin rule above.

This is registered because **the UK boundary rests on two kinds of evidence and, until now, only one of them said which it was.** The corpus share behind it — 44,830 zones, 32.3% — is measured, against `zones-dump-2025-01-04.json`, and cited to that dump. The speed figures are not, and they are the half deciding whether UK rural roads exceed the maximum at all. Neither half is evidence for the maximum itself, which carries no origin and is deliberately not enumerated among the constants above.

**Conversions from mph are rounded to one decimal place**, at 1 mph = 1.609344 km/h: 30 mph is 48.3 km/h, 60 mph is 96.6, and 70 mph is 112.7. Rounded, not truncated — 70 mph is 112.65408 km/h, which truncates to 112.6 and rounds to 112.7, and it had been written truncated alongside 96.6 rounded in the same sentence. No verdict anywhere turns on the difference, since every figure above the maximum clears it by tens of km/h; the convention is stated so that two conversions standing side by side are not performed two different ways.
