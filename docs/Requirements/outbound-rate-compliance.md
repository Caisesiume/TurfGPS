...
Statement:    The system shall issue no more requests to a Turf API resource
              ...
              *Retrieving zones* in `Architecture.md` permit, counting every
...
Verification: test — several solve sessions running alongside a scheduled sync
              ...
              Turf resource exceeds the limits that section states
Acceptance:   outbound requests to a Turf resource per unit time ≤ the
              per-resource limit under `Architecture.md § Retrieving zones`,
              sustained while several solve sessions and a scheduled sync run
              together
...
Risk:         The Turf API is the only source of zone, player and region data,
              ...
              product. Nor can a breach be retried away: at the interval that
              section states, a wasted request costs the next window rather
              than a moment.
