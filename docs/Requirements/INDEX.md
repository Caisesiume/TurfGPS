...
| [Hand-off and dispatch](#hand-off-and-dispatch) | `hand-off-and-dispatch.md` | `FR-109` – `FR-115` (7) |
| [Persistence and staleness](#persistence-and-staleness) | `persistence-and-staleness.md` | `FR-094` – `FR-108`, `FR-116` (16) |
...
| `FR-110` | Size a dispatched portion from the intermediate-waypoint allowance | Hand-off and dispatch | MUST | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-094` | Keep a confirmed plan reopenable after an arbitrary interval | Persistence and staleness | MUST | to-build | test | — |
...
**The split across the corpus is 119 `test` · 7 `inspection` · 4 `human-judgement`**, which is 130. Batch 6 added twenty-seven `test` and three `inspection` on 28 August 2026 — `FR-109`, `NFR-010` and `NFR-011` — and **added no `human-judgement`, which is the first batch since batch 3b to return that zero and the first to return it at this size.** The look was run per record as the skill requires; what the batch offers is discipline rather than quality — a key that must not be guessable, a code that must not reach a log, a row that must be gone — and each has a crisp pass/fail with no standard of judgement in it, so the judged method was never in play. The three `inspection` records are chosen methods and not tests not yet written: `FR-109` and `NFR-011` examine a configuration for figures that must not be literals in code, and `NFR-010` examines a code path from its entropy source to the value written, which no black-box run can distinguish from a well-disguised counter. Batch 5 added thirty-six `test` and two `human-judgement` on 7 August 2026 — `FR-059` and `NFR-007` — and is the first batch to file more than one judged record, doubling the corpus's count of them. It added no `inspection`. Batch 4 added sixteen `test` and one `human-judgement` on 6 August 2026, and `FR-055` … `FR-057` three further `test` the same day. `FR-055`'s `Verification` was amended twice on 7 August 2026 — its second half before the record's signature and its first half after — and **its keyword moved on neither**, so the split is untouched by both: this column carries the keyword alone, and a change to what the method proves leaves no trace here by design. The record is where that sentence is read, and `README.md § ID allocation ledger` is where the two amendments are told apart.
