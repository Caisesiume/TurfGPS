...
| [Hand-off and dispatch](#hand-off-and-dispatch) | `hand-off-and-dispatch.md` | `FR-109` – `FR-115`, `FR-118` (8) |
| [Persistence and staleness](#persistence-and-staleness) | `persistence-and-staleness.md` | `FR-094` – `FR-108`, `FR-116`, `FR-117` (17) |
...
| [Privacy](#privacy) | `privacy.md` | `NFR-008` – `NFR-018` (11) |
| [Runtime and deployment shape](#runtime-and-deployment-shape) | `runtime-and-deployment-shape.md` | `NFR-003` – `NFR-005` (3) |

**136 requirements, in 23 categories.** Sections below follow the register's order in `README.md`, not the ID order — a category's records are contiguous in its own file, never across the corpus. A category on the register with no records filed yet has no file and no section here; the register in `README.md` is the full list of legal names, and a name's absence from this page means unfiled, never illegal.

**All 136 rows read `to-build`, and no row reads `draft`.** Batch 6 — `FR-094` … `FR-118` and `NFR-008` … `NFR-018` — was authored, de-conflicted and moved `draft` → `to-build` by `@requirements-engineer` under `ADR-0001 § D6` before it was filed on 28 August 2026, its remaining questions logged in `DECISIONS.md` as `RD-010` … `RD-040` — **thirty-one entries**, seventeen at the filing pass and the rest added by the three revisions PR #135's review produced, which `README.md § ID allocation ledger` blocks and dates; it is the first batch this index has taken whose rows have never held `draft` in a committed state. Batch 5 — `FR-058` … `FR-093`, `NFR-006` and `NFR-007` — was filed on 7 August 2026 and moved `draft` → `to-build` on 14 August 2026, `@requirements-engineer` recording the transition itself under `ADR-0001 § D6` with the batch's seven remaining questions logged in `DECISIONS.md`. Every row filed before it moved the same way: batch 4 — `FR-038` … `FR-054` — was signed off on 6 August 2026 and all seventeen moved from `draft` **straight to `to-build`**, as batch 3b did earlier the same day, as batch 3 did on 4 August 2026 and as batches 1 and 2 each did on the day they were filed, and `FR-055` … `FR-057` moved the same way on 7 August 2026. `approved` stays unreachable while `@requirements-reconciler` is dormant, and no record in this corpus has ever held it. **Nothing in this column says how far the build has got** — that question is answered by following `Resolved-by` to the stories, never by a value written here. See `README.md § Corpus state`.

**62 rows name at least one story in `Resolved-by`, and 74 read `—`.** The sixty-two were allocated in six passes — eighteen on 2 August 2026, the twenty-one batch-3 rows on 4 August 2026, the three batch-3b rows and seventeen batch-4 rows on 6 August 2026, and three more on 7 August 2026, each allocation following its batch's sign-off as a pass of its own — and thirteen epics and fifty-nine stories carry them. **The rows naming a story and the rows reading `to-build` are no longer the same set**, which is the visible consequence of the transitions above: all 136 read `to-build` and seventy-four of them name no story yet. `—` says *not yet allocated* here and never *unknown*, and **the seventy-four are not one population.** Batch 5's thirty-eight are a story pass that is **due**: story creation follows `to-build` and not the reverse (`ADR-0001 § D6`), so what gated them has happened. Batch 6's thirty-six are a story pass that is **not due at all**, under the Owner's sequencing directive that the platform is specified before stories are made — their `—` is deliberate rather than pending, and `README.md § ID allocation ledger` records it as the third reading this column's em-dash has carried. Which stories, and which epic each requirement sits under, is `TRACEABILITY.md`; this column is a view of its first table.
...
| `FR-058` | Generate more than one general road route | Route alternative generation | MUST | to-build | test | — |
| `FR-059` | Produce general road routes that are different drives | Route alternative generation | MUST | to-build | human-judgement | — |
| `FR-060` | Build on more than the fastest conventional route | Route alternative generation | SHOULD | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-063` | Bound a corridor set by the corridor half-width | Candidate identification | MUST | to-build | test | — |
| `FR-064` | Promote no more candidates to full evaluation than the cap | Candidate identification | MUST | to-build | test | — |
| `FR-065` | Record where the promotion cap bound | Candidate identification | MUST | to-build | test | — |
| `FR-080` | Apply the configured maximum walking distance as a bound | Candidate identification | MUST | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-067` | Assign exactly one access class to every classified candidate | Access classification | MUST | to-build | test | — |
| `FR-068` | Choose the validation regime from the direct-access tolerance | Access classification | MUST | to-build | test | — |
| `FR-071` | Send an established level or barrier failure to the park-and-walk branch | Access classification | SHOULD | to-build | test | — |
| `FR-072` | Classify uncertain where the direct-access evidence is ambiguous | Access classification | MUST | to-build | test | — |
| `FR-073` | Measure a candidate's access route to the zone's coordinate | Access classification | MUST | to-build | test | — |
| `FR-075` | Build no access path from a way the data bars to pedestrians | Access classification | MUST | to-build | test | — |
| `FR-078` | Validate every leg of a multi-zone walking route | Access classification | MUST | to-build | test | — |
| `FR-081` | Assign a confidence level to every access estimate | Access classification | MUST | to-build | test | — |
| `FR-082` | Classify a low-confidence access estimate as uncertain | Access classification | MUST | to-build | test | — |
| `FR-083` | Admit park-and-walk only on a routed path and an obtained elevation profile | Access classification | MUST | to-build | test | — |
| `FR-084` | Keep uncertain candidates out of the optimization | Access classification | MUST | to-build | test | — |
| `FR-085` | Classify a zone accessible only on an identified connection | Access classification | MUST | to-build | test | — |
| `FR-092` | Identify a stopping position rather than manufacture one | Access classification | MUST | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-086` | Route a detour's cost through the proposed stopping position | Route construction | MUST | to-build | test | — |
| `FR-087` | Reuse no candidate's detour cost across journeys | Route construction | MUST | to-build | test | — |
...
| `FR-062` | Keep the general-route deviation separable from the Turf stop time | Cost and time composition | SHOULD | to-build | test | — |
| `FR-074` | Compute access cost from the routed path and its elevation profile | Cost and time composition | MUST | to-build | test | — |
| `FR-079` | Price a park-and-walk stop over the set of zones it serves | Cost and time composition | MUST | to-build | test | — |
| `FR-093` | Compute both halves of a stop against one resolved stopping position | Cost and time composition | MUST | to-build | test | — |
...
| `FR-066` | Tell the user where the promotion cap shaped the result | Recommendation disclosure | SHOULD | to-build | test | — |
| `FR-088` | State the material uncertainty in an access estimate | Recommendation disclosure | SHOULD | to-build | test | — |
...
`hand-off-and-dispatch.md`

| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-109` | Hold each hand-off target's waypoint limits as configuration | Hand-off and dispatch | MUST | to-build | inspection | — |
| `FR-110` | Size the first dispatch from the intermediate-waypoint allowance | Hand-off and dispatch | MUST | to-build | test | — |
| `FR-111` | Carry the portion's last Turf stop in the destination slot | Hand-off and dispatch | SHOULD | to-build | test | — |
| `FR-112` | Reduce a refused hand-off rather than abandon it | Hand-off and dispatch | MUST | to-build | test | — |
| `FR-113` | Tell the user what a hand-off may drop, before it is made | Hand-off and dispatch | MUST | to-build | test | — |
| `FR-114` | Leave the stored plan unchanged by a hand-off | Hand-off and dispatch | MUST | to-build | test | — |
| `FR-115` | Offer hand-off only to the target named for the first release | Hand-off and dispatch | MUST | to-build | test | — |
| `FR-118` | Dispatch each Turf stop at its stopping position | Hand-off and dispatch | MUST | to-build | test | — |

## Persistence and staleness

`persistence-and-staleness.md`

| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `FR-094` | Keep a confirmed plan reopenable within its retention bound | Persistence and staleness | MUST | to-build | test | — |
| `FR-095` | Add a new stored plan without displacing an existing one | Persistence and staleness | MUST | to-build | test | — |
| `FR-096` | Retain with a stored plan the state a later re-solve needs | Persistence and staleness | MUST | to-build | test | — |
| `FR-097` | Judge a later review against the allowance retained with the plan | Persistence and staleness | MUST | to-build | test | — |
| `FR-098` | Hold no zone ownership in a stored plan | Persistence and staleness | MUST | to-build | test | — |
| `FR-099` | Record with each stored plan when it was computed | Persistence and staleness | MUST | to-build | test | — |
| `FR-100` | Require no identity to retrieve a plan by its plan code | Persistence and staleness | MUST | to-build | test | — |
| `FR-101` | Make a stored plan's plan code available from its list entry | Persistence and staleness | SHOULD | to-build | test | — |
| `FR-102` | Identify each listed stored plan by origin, destination and date | Persistence and staleness | MUST | to-build | test | — |
| `FR-103` | Answer an expired plan code and an unrecognised one alike | Persistence and staleness | MUST | to-build | test | — |
| `FR-104` | Extend a stored plan's rolling retention within its ceiling | Persistence and staleness | MUST | to-build | test | — |
| `FR-105` | Open a stored plan without the wizard and without the Turf API | Persistence and staleness | MUST | to-build | test | — |
| `FR-106` | Present a reopened stored plan exactly as it was confirmed | Persistence and staleness | MUST | to-build | test | — |
...
| `FR-061` | Test the stated limit and the ceiling against the whole additional time | Safety exclusions | MUST | to-build | test | — |
| `FR-069` | Reject a direct-access tolerance above the permitted value | Safety exclusions | MUST | to-build | test | — |
| `FR-070` | Require compatible levels and no barrier for direct road access | Safety exclusions | MUST | to-build | test | — |
| `FR-076` | Exclude an absent, disconnected, or implausibly steep access path | Safety exclusions | MUST | to-build | test | — |
| `FR-077` | Let no zone's value admit an unsafe or infeasible access route | Safety exclusions | MUST | to-build | test | — |
| `FR-089` | Propose no stop on a road the exclusions refuse | Safety exclusions | MUST | to-build | test | — |
| `FR-090` | Reject a maximum stopping speed above the permitted value | Safety exclusions | MUST | to-build | test | — |
| `FR-091` | Plan no journey while an owed enforcement constant is unconfigured | Safety exclusions | MUST | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `NFR-006` | Fall access confidence with the evidence behind the estimate | Coverage and data quality | MUST | to-build | test | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `NFR-007` | Classify no zone confidently and wrongly | Classification correctness | MUST | to-build | human-judgement | — |
...
| ID | Title | Category | Priority | Status | Verification | Resolved-by |
|---|---|---|---|---|---|---|
| `NFR-008` | Keep the plan code out of every log the service writes | Privacy | MUST | to-build | test | — |
| `NFR-009` | Hold no Turf username in a stored plan | Privacy | MUST | to-build | test | — |
| `NFR-010` | Make a plan code unpredictable | Privacy | MUST | to-build | inspection | — |
| `NFR-011` | Read the plan code's length and alphabet from configuration | Privacy | MUST | to-build | inspection | — |
| `NFR-012` | Rate-limit plan retrieval per caller | Privacy | MUST | to-build | test | — |
| `NFR-013` | Bound a stored plan's retention by both clocks | Privacy | MUST | to-build | test | — |
| `NFR-014` | Delete a stored plan once its expiry has passed | Privacy | MUST | to-build | test | — |
| `NFR-015` | Keep the plan code out of every log the reverse proxy writes | Privacy | MUST | to-build | inspection | — |
| `NFR-016` | Record a retrieval attempt without recording the plan code | Privacy | MUST | to-build | inspection | — |
| `NFR-017` | Read the per-caller retrieval limit and window from configuration | Privacy | MUST | to-build | inspection | — |
| `NFR-018` | Bound what a hand-off carries to the target application | Privacy | MUST | to-build | test | — |
...
**The split across the corpus is 122 `test` · 10 `inspection` · 4 `human-judgement`**, which is 136. Batch 6 added thirty `test` and six `inspection` on 28 August 2026 — `FR-109`, `NFR-010`, `NFR-011`, `NFR-015`, `NFR-016` and `NFR-017` — and **added no `human-judgement`, which is the first batch since batch 3b to return that zero and the first to return it at this size.** The look was run per record as the skill requires; what the batch offers is discipline rather than quality — a key that must not be guessable, a code that must not reach a log, a row that must be gone — and each has a crisp pass/fail with no standard of judgement in it, so the judged method was never in play. The six `inspection` records are chosen methods and not tests not yet written, and the revision's three enlarge the reason rather than repeat it: `FR-109` and `NFR-011` examine a configuration for figures that must not be literals in code and `NFR-017` does the same for the per-caller limit and its window; `NFR-010` examines a code path from its entropy source to the value written, which no black-box run can distinguish from a well-disguised counter; `NFR-015` examines a reverse-proxy configuration whose defect is an **absent** directive, which a run against a proxy configured for the run cannot show; and `NFR-016` examines the derivation producing an attempt's identifier, irreversibility being a property of how a value was made and not one any single value exhibits. Batch 5 added thirty-six `test` and two `human-judgement` on 7 August 2026 — `FR-059` and `NFR-007` — and is the first batch to file more than one judged record, doubling the corpus's count of them. It added no `inspection`. Batch 4 added sixteen `test` and one `human-judgement` on 6 August 2026, and `FR-055` … `FR-057` three further `test` the same day. `FR-055`'s `Verification` was amended twice on 7 August 2026 — its second half before the record's signature and its first half after — and **its keyword moved on neither**, so the split is untouched by both: this column carries the keyword alone, and a change to what the method proves leaves no trace here by design. The record is where that sentence is read, and `README.md § ID allocation ledger` is where the two amendments are told apart.
