...
**`RD-027` is the first exercise, and on the day this was written the only one.** Its `Decided-by:` gained `#issuecomment-5457129534` because that comment directs, in its own text, that the entry be recorded citing it — so the entry as first written did not yet obey the ruling it records. Nothing it asserts was touched. `README.md § ID allocation ledger` carries the record of that edit, and this paragraph names it so a reader of this file alone meets it rather than only a reader of `git`. Ruled on PR #135 cycle 4, on `DOC-18`: that lane found this section's literal text permitting what glosses of it elsewhere in this corpus, and the opening of issue #126, forbade flatly, and declined to revert an edit it judged substantively right. **A rule whose two readings disagree is the defect**, whichever reading a given pass happens to take.
