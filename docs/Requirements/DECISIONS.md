...
**That rule's boundary runs between what an entry *asserts* and the provenance of what it records, and it is stated here because an edit has now crossed it.** What an entry asserts — its `Interpretation`, its `Rung`, the question it answers, the records it binds — is **superseded by a new entry and never repaired in place**, which is the paragraph above and admits no exception. **An additive provenance citation that the recorded ruling itself directs is permitted in place**, because it changes nothing the entry asserts and supplies only the reference the ruling asked the entry to carry. A log that could not obey such a direction without superseding the entry would answer a ruling *about* provenance by making that entry's provenance harder to read, and the superseding entry would record no decision — which is the one thing an entry in this file exists to do.

**The permission is narrow by construction: additive only, provenance only, and only where the cited ruling directs the citation.** Each of the three is load-bearing. *Additive* excludes rewording what is already there. *Provenance only* excludes every field a reader weighs the decision by. *Directed by the ruling* is what keeps the entry's author out of it — the entry gains a citation because the ruling it records asked for one, never because its author later thought better of the entry. **An edit that both adds provenance and alters an assertion is an alteration**, and it is superseded like any other. Where the direction is arguable rather than in the ruling's own text, the entry is superseded instead: the narrow reading costs one extra entry and the wide one costs this log its evidential value.

**`RD-027` is the first exercise, and on the day this was written the only one.** Its `Decided-by:` gained `#issuecomment-5457129534` because that comment directs, in its own text, that the entry be recorded citing it — so the entry as first written did not yet obey the ruling it records. Nothing it asserts was touched. `README.md § ID allocation ledger` carries the record of that edit, and this paragraph names it so a reader of this file alone meets it rather than only a reader of `git`. Ruled on PR #135 cycle 4, on `DOC-18` and `SAFE-12`: both lanes found this section's literal text permitting what six glosses of it elsewhere in this corpus, and a seventh opening issue #126, forbade flatly — `README.md § ID allocation ledger` names all seven — and both declined to revert an edit they judged substantively right. **A rule whose two readings disagree is the defect**, whichever reading a given pass happens to take.
