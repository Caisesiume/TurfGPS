...
**Corpus state — 2 August 2026.** 18 requirements, 5 categories in use. **All eighteen are `to-build`.** `FR-001` … `FR-012` were signed off by the Owner on 1 August 2026 and `FR-013` … `FR-018` on 2 August 2026; each sign-off moved every record it covered from `draft` **straight to `to-build`**, because `@requirements-reconciler` is dormant and there is no classification step for a record to wait in `approved` for — no record in this corpus has ever held `approved`. Story creation then ran for the first twelve: `@requirements-story-organizer` filed three epics and eleven stories on 2 August 2026, and each of the twelve is resolved by a story. The links live in `TRACEABILITY.md`; each record's `Resolved-by` and the column in `INDEX.md` are views of them.

**`FR-013` … `FR-018` are batch 2, filed 2 August 2026 and signed off the same day.** All six read `Status: to-build` and all six read `Resolved-by: —`: story creation follows sign-off as a pass of its own rather than sharing it, exactly as it did for batch 1. Six em-dashes in the `Resolved-by` column are therefore *not yet allocated* and not orphans — the orphan test applies to a requirement whose epic is in flight, and these have no epic yet.
