...
**Corpus state — 2 August 2026.** 18 requirements, 5 categories in use. Twelve are `to-build` and six are `draft`. `FR-001` … `FR-012` were signed off by the Owner on 1 August 2026; that sign-off moved every one of them from `draft` **straight to `to-build`**, because `@requirements-reconciler` is dormant and there is no classification step for a record to wait in `approved` for — no record in this corpus has ever held `approved`. Story creation then ran for those twelve: `@requirements-story-organizer` filed three epics and eleven stories on 2 August 2026, and each of the twelve is resolved by a story. The links live in `TRACEABILITY.md`; each record's `Resolved-by` and the column in `INDEX.md` are views of them.

**`FR-013` … `FR-018` are batch 2, filed 2 August 2026 and not yet signed off.** All six read `Status: draft`, all six read `Resolved-by: —`, and no story or epic exists for any of them — `@requirements-story-organizer` is deliberately not invoked until sign-off, exactly as batch 1 was. Six em-dashes in the `Resolved-by` column are therefore *not yet allocated* and not orphans, and nothing in the corpus may be read as approved on their account.
