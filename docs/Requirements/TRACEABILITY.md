...
42 of 42 requirements listed. **All forty-two carry an epic and at least one story** — nine epics and thirty-nine stories, allocated in four passes — and no row reads `—` under either column. The rule stays one row per non-retired requirement rather than one row per requirement that has a story: a requirement missing from this table is a filing error whatever its status, and a row added only once a story exists would make the table's completeness unverifiable exactly when it matters. **The rule has now been exercised rather than only stated**, and that is worth keeping once the rows it carried are filled: between batch 3b's filing and its story allocation, both on 6 August 2026, `FR-035` … `FR-037` were the first three rows this table had ever held with no story behind them, and they were the case the rule was written for.

**The orphan rule is scoped to an epic in flight**: a requirement reading `—` under *Stories* while its epic is in flight is an orphan and a finding, and a requirement with no epic at all has not reached that test yet. **No row reads `—` today, so the rule has nothing to bite on** — it stands against the next requirement filed ahead of its allocation, which is the ordinary interval between a filing pass and the story pass that follows it, and it bites on the first such requirement to still read `—` under *Stories* once its epic is cut. `FR-035` … `FR-037` held that shape for part of 6 August 2026 and none of the three was ever an orphan: each had no epic at all, so none reached the test before its epic was named. Every requirement code here that names a story appears in the second table below, and every code there appears here — a code with a story in one direction and no counterpart in the other is a finding. A requirement with no story is absent from that table by construction and is not that finding.
