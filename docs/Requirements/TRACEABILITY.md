...
62 of 62 requirements listed. **Fifty-nine carry an epic and at least one story** — thirteen epics and fifty-six stories, allocated in five passes — and **three read `—` under both columns**: `FR-055` … `FR-057`, filed `draft` on 6 August 2026 and not yet signed. The rule stays one row per non-retired requirement rather than one row per requirement that has a story: a requirement missing from this table is a filing error whatever its status, and a row added only once a story exists would make the table's completeness unverifiable exactly when it matters. **The rule was first exercised by batch 3b** — between its filing and its story allocation, both on 6 August 2026, `FR-035` … `FR-037` were the first three rows this table had ever held with no story behind them — and batch 4 was the second and much larger instance of the same shape, and the first in which the requirements were **signed off** rather than `draft`. Both intervals are closed and both were the length of a pass. The three rows still reading `—` are `draft`, which is the earlier and more ordinary of the two reasons.

**The orphan rule is scoped to an epic in flight**: a requirement reading `—` under *Stories* while its epic is in flight is an orphan and a finding, and a requirement with no epic at all has not reached that test yet. **Three rows read `—` today and none of them is an orphan**, because each also reads `—` under *Epic (Milestone)* — no epic has been cut for `FR-055` … `FR-057`, so none of the three has reached the test. The rule bites on the first of them to still read `—` under *Stories* once its epic is cut. **Sign-off does not bite either, and the point survives the case that prompted it**: the test is an epic in flight, not a signature, so a signed-off requirement with no epic is exactly as far from being an orphan as a `draft` one. `FR-035` … `FR-037` held that shape for part of 6 August 2026 and `FR-038` … `FR-054` for another part of the same day, and none of the twenty was ever an orphan, for the same reason. Every requirement code here that names a story appears in the second table below, and every code there appears here — a code with a story in one direction and no counterpart in the other is a finding. A requirement with no story is absent from that table by construction and is not that finding.
