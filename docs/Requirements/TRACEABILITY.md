...
**Batch 3b — filed 6 August 2026, no stories.** `FR-035` … `FR-037` were filed on 6 August 2026 and are `draft`, awaiting the Owner's sign-off. Story creation follows sign-off as a pass of its own, so neither an epic nor a story exists for any of the three, and all three read `—` under both columns of the first table — the first `—` in this file, and in the corpus, since story creation caught up with filing on 4 August 2026. It carries its one meaning: *not yet allocated*, never *unknown*. Nothing goes in the second table for them, which is not an omission — that table is transcribed from issues' `Resolves:` lines, and there are no issues.
...
42 of 42 requirements listed. **Thirty-nine carry an epic and at least one story** — nine epics and thirty-six stories, allocated in three passes — and **three, `FR-035` … `FR-037`, read `—` under both columns**, filed after the last allocation and not yet signed off. The rule stays one row per non-retired requirement rather than one row per requirement that has a story: a requirement missing from this table is a filing error whatever its status, and a row added only once a story exists would make the table's completeness unverifiable exactly when it matters. Those three rows are the first the rule has had to carry, and they are the case it was written for.

**The orphan rule is scoped to an epic in flight**: a requirement reading `—` under *Stories* while its epic is in flight is an orphan and a finding, and a requirement with no epic at all has not reached that test yet. **`FR-035` … `FR-037` read `—` under both columns and none of the three is an orphan**: each has no epic at all, so none has reached that test. The rule bites on the first of them to still read `—` under *Stories* once its epic is cut. Every requirement code here that names a story appears in the second table below, and every code there appears here — a code with a story in one direction and no counterpart in the other is a finding. A requirement with no story is absent from that table by construction and is not that finding.
