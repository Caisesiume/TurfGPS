...
Every document named in `§ The documents` above exists.
...
- **`SPECIFICATION.md`** is the source of truth for intent. The implementation has only begun — `Architecture.md § D8` records what `service/` landing changed and what it did not — so the documents still lead the implementation rather than the other way round.
...
**A citation is one self-contained token, and the `§` sits inside its delimiters.** A cross-document citation names its target within the delimiters — *SPECIFICATION.md § Park-and-walk zones* — and a same-file citation omits the target and opens on the marker — *§ Current state*. **Both examples are the form the rule requires, and neither reports how any file is currently written.** The rule binds every file, and what any given file currently holds lives in the converted-file list below: a file on that list has had every citation checked token-form, and a file absent from it is one the list says nothing about — never one the rule waits on. The marker being inside is the whole of the design: **a delimited span containing `§` is a citation, and one without it is emphasis.** The two are then decidable by looking at the span — no list of headings to consult, and no filter to separate citations from the ordinary emphasis sharing their delimiter. It is also what makes a bare cross-document citation *unwritable* rather than merely discouraged: the target-less form already means *this file*, so no spelling is left over for "some other document, unnamed".
...
**Until 7 August 2026 the rule above was written *as the four documents write it*, and that was false.** Measured on that day and against the commit that repaired it, those four held 112 cross-document citations in the superseded form against 10 in the token, so a reader checking the convention against the documents found it contradicted — and the reading available to them was that the *rule* was aspirational rather than that the *files* were unmigrated. That is the direction a convention fails in when it asserts a state instead of stating a rule, and the failure is invisible from the sentence itself: nothing in it goes false on a day anyone is editing it. What it was reaching for is left to the converted-file list below, which records that state one file at a time and only on a check that was actually run.
...
**An agent definition is cited the same way, by the `name` in its own frontmatter** — `requirements-engineer § Mode A` — and **that name resolves by convention to `.claude/agents/<name>.md`**. The parity is the argument rather than an analogy: an agent's `name` is its invocation key exactly as a skill's is, it is the field every dispatch addresses, and it survives the file moving, while the fixed directory and extension around it carry nothing the name does not. So the reasoning that makes a skill's name the durable half of the token makes the filename the volatile half here too. Ruled by `@engineering-lead` on 7 August 2026, after `@docs-writer` wrote the repository's first citation of an agent definition in the filename form and flagged it before it multiplied; the one instance then in existence was converted with the ruling.

**A target written as a location rather than as its name is one defect and not two.** A skill cited by its path and an agent cited by its filename are the same shape wearing the two spellings their two directories happen to offer, and both are defects on sight. Stating the class here rather than the pair matters because the class is what an enumeration elsewhere would have to keep chasing: `local-gates § Documentation gates` names the shapes it refuses and cites this section for the rule, so widening the rule here is what reaches it, and a third spelling arriving with some future kind of target is already covered.

**Where a skill and an agent ever share a name, the skill keeps the bare form and the agent is written `@name § Section`.** No pair collides today — every `name` under `.claude/agents/` was checked against every `name` under `.claude/skills/` on 7 August 2026 — and the rule is recorded anyway, because a collision is discovered at the moment somebody is naming a new agent, which is the worst moment to be inventing a convention and the moment at which whatever gets invented becomes the precedent. The `@` is not new syntax: it is how every dispatch in this repository already addresses an agent, so it is the disambiguation a reader guesses rather than one they must look up. The skill keeps the bare form because skills are cited far more often, and by files that dispatch nothing. **Not colliding is still better than resolving a collision** — a shared name costs every reader of both a moment's doubt, whichever form is written — so this settles what such a citation means, never that the naming was a good idea.
...
**A citation rots when the *cited* file changes, and nothing in the citing file moves.** No diff, no failing pattern, no count: `FR-022` and `NFR-001` were each true when written and false when read, and nothing in either record could have told a reader so. **The check is a heading lookup, not a pattern**, and it asks two questions — does the cited heading still exist, and does that section still hold the claim the citing line attributes to it. A rename fails the first; **a re-split fails only the second**, the heading surviving while the content moves out from under it, which is why heading-existence alone is not the check.

**The duty is primary on whoever edits the cited file, and it reaches every file in this repository.** A commit that renames a heading, re-splits a section, or retracts what one asserted greps the repository for citations naming it and repairs them **in that same commit** — the only moment at which both the old name and the new one are known, and one that no reader of the citing file ever reaches. It is recorded here rather than in `requirements-authoring`, where it was written and where it read as a corpus rule, because the defect is a property of citations and this repository's citations are not confined to the corpus: skill files cite skill files, agent definitions cite skills, and a duty scoped to `docs/Requirements/` says nothing about any of them.

**The backstop is `@requirements-librarian`'s, and it did not generalise with the primary duty.** The librarian re-resolves the corpus's outbound citations against their cited files' current headings **on every pass**, because the editor may be an agent that never loads `requirements-authoring`. That is a sweep over `docs/Requirements/` and over nothing else — **no librarian was ever going to reach a skill file**, which is exactly where the defect prompting this move was living. Stating the limit is the point and not a caveat: generalising the primary duty while leaving the backstop's scope unsaid would read as cover for the files nothing sweeps, and the hole would be invisible precisely where it is real. **Neither party discharges the other** — the backstop catches what the primary missed, and catches it only after an interval in which the citation was read as true — and outside the corpus there is no second party at all, which is what makes the primary duty load-bearing rather than merely first.

**Enforcement is gate 1, and that is the resolution rather than a compromise.** A gate fires on a PR's diff, and the party editing the cited file *is* the party opening that PR — so the rename or the re-split is in their diff, and the repairs this duty asks for are in that same diff or are visibly absent from it. Both halves of the obligation sit in one place at the one moment it is owed, which no sweep run later can offer. `local-gates § Documentation gates` carries the clause and cites this section rather than restating it.
...
**Every other file takes the code span, and a property decides it rather than a tally.** This file, `DELIVERY.md`, `HANDOFF.md`, `DEPLOYMENT.md`, the corpus's own `README.md`, `INDEX.md` and `TRACEABILITY.md`, the skill files under `.claude/skills/`, and the agent definitions under `.claude/agents/` are all read rendered, and none is one of the four documents, so neither rendering's stated reason reaches them. What does reach them is how they are used: **these are working documents, consulted rather than read through.** A reader arrives at one by searching for a term, or by loading it whole into an agent's context — never front to back — so the criterion that settles the corpus's record fields, that a citation be findable by `grep`, applies here in its own right rather than because a fence suppresses rendering. That is the third class, and its delimiter is the code span.

**A specimen shows the form it demonstrates, and the delimiter rule binds uses rather than mentions.** A citation written to *be* a citation carries its own file's delimiter. A citation written to **show a reader what a form looks like** carries the form it is showing, wherever it appears. This file is the proof: the filename rule above writes *Architecture.md § D1* in italics, inside a working document whose delimiter is the code span, and it has to. What that rule demonstrates is that a filename inside a citation carries the citation's delimiter and no other — so a code span there would nest the exact thing the line forbids and destroy the failure mode it exists to show. The same holds for the superseded form quoted above, for the two examples in the opening rule, and for every other specimen in this section: each is a quotation of a shape, not a reference to a heading. **The marker mentioned as a character is the same case and is the one a checker actually trips on** — the opening rule writes `§` in a code span, which is a delimited span containing the marker and therefore a citation by the pattern above, resolving to a section whose name is the empty string. It is a mention of a character. A resolver that reports it has found a defect in itself.

**Gate 1's delimiter-conformance part therefore does not fire on a specimen**, and `local-gates § Documentation gates` is checking a file's own citations rather than its illustrations of anyone else's. Re-delimiting a specimen to match its host file is a category error that silently deletes the thing being shown. **The other parts of gate 1 are unaffected**, which is the limit of this rule and not a caveat on it: a specimen still names a real document and a real heading wherever it names one, *Architecture.md § D1* resolves exactly as any citation of `D1` would, and a specimen invented to illustrate a form against a heading that does not exist is a defect like any other. Ruled by `@engineering-lead` on 7 August 2026 as a mechanical consequence of the delimiter rule already recorded here, touching no safety path; the Owner can overturn it.
