...
- **A claiming lane is claimed before it is dispatched.** The judge writes every selected claiming lane into the table first, and such a lane with no claim row is not convened. A second dispatch against a claimed lane at the same head SHA is **refused**, not discouraged.
...
- **The judge synthesises by reading the table**, against the set it recorded before it claimed anything. Completeness is a property of the table and never of what happened to come back. That is what makes the absence of a verdict distinguishable from the absence of a dispatch, which is the confusion that put several judges on one PR — and recording the selection is what stops *complete* from meaning complete against whatever a dying judge managed to claim.

**Not every convened lane is a claiming lane, and the boundary is stated rather than inferred.** The table covers lanes that produce a ruling; a lane convened to weigh other lanes' verdicts, or to set a risk tier, produces none and does not claim. Which lanes fall on which side is one roster, in `review-board-dispatch § The claim table covers verdict-producing reviewer lanes only`, and it is not restated here. **Completeness is computed over the claimed set precisely so a non-claiming lane cannot deadlock a panel** — the unstated boundary did exactly that, permanently, at every high tier.
