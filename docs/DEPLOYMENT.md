...
**Nothing described here has been deployed, and none of the configuration named below exists on disk.** The repository holds `service/`; it holds no `web/` and no `deploy/` as this is written, 22 August 2026. This document is a design, in the same sense `Architecture.md` is one — it is read as *what the deployment will be*, not as a record of a running system. **That date is this document's as-of date everywhere but one place**: everything it states about what is true now rather than what it specifies is measured then, and exactly one paragraph carries a date of its own — the record under `§ Where the deployment configuration lives` of the environment the service reads, measured on 28 August 2026 because the code it describes landed after this preamble was written. That paragraph is also the one exception to this paragraph's opening sentence: what the service reads is on disk, while the configuration that would satisfy it still is not.
...
**The service reads two environment variables at start-up, and they are the whole of its configuration surface.** They are new, and this paragraph is measured on 28 August 2026 rather than at this document's as-of date above: `service/internal/config/owed.go`, called from `service/cmd/turfgps/main.go`, introduced them. Each supplies one enforcement constant that `CalculationSpecification.md § Enforcement constants that do not yet exist` records as owed.
...
**Both constants are owed and unauthored, and no value for either may be written here or anywhere under `deploy/`.** Neither has a value in any document. Which constants are owed, why each is, which direction each may only be revised in, and what each gates while unset are stated once, in the section cited above; this document records only the deployment-facing half — that a value arrives through the environment, and what the service does when one does not. Neither constant is a proposed default awaiting evidence, and neither may be given a plausible figure to get a deployment running. `docs/Requirements/safety-exclusions.md § FR-091` is the requirement the refusal discharges.

**The check is presence and never shape, and it runs ahead of the listener.** A variable that is unset, or set to a blank value, carries no value. The service then logs every constant it found no value for — all of them, rather than the first — and exits non-zero without binding a port, so a refused configuration serves nothing at all rather than serving requests it cannot plan against. Nothing is parsed: what a value must look like is not settled in any document, and settling it here would be code deciding a specification question.
