...
**Both constants are owed and unauthored, and no value for either may be written here or anywhere under `deploy/`.** Neither has a value in any document. Which constants are owed, why each is, and which direction each may only be revised in are stated once, in the section cited above; this document records only the deployment-facing half — that a value arrives through the environment, and what the service does when one does not. **What follows from either being unset is not stated here for either constant.** It is stated in the section cited above and in the `Risk` field of `docs/Requirements/safety-exclusions.md § FR-091`, which is also the requirement the refusal discharges, and a reader who needs it reads both because neither answers for the other. **What this document does state is narrower, and it is the same for both**: unset, the service does not start. That is the refusal's doing rather than anything either constant gates on its own, and nothing here should be read as attributing a blocking rule to either. Neither constant is a proposed default awaiting evidence, and neither may be given a plausible figure to get a deployment running.

**"Unauthored" and "unset" are not the same state, and only the second is what the service checks.** A constant is unauthored while no document gives it a value — the state both are in, and the state the prohibition above is about. A variable is unset while the environment carries no value for it, which is what the check below tests. So **any non-blank string satisfies that check**: a placeholder gets a host past start-up with the constant still unauthored, and nothing anywhere reports that it did. A service that started is therefore evidence that both variables were set and never evidence that either constant was authored. That gap is what the prohibition is for, and why it is worded against supplying a value rather than against omitting one.
