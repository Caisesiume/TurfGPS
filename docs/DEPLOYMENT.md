...
**Nothing described here has been deployed, and the configuration named below is almost entirely unwritten.** The repository holds `service/`; it holds no `web/` and no `deploy/` as this is written, 22 August 2026. This document is a design, in the same sense `Architecture.md` is one — it is read as *what the deployment will be*, not as a record of a running system. **That date is this document's as-of date everywhere but one run of paragraphs**: everything it states about what is true now rather than what it specifies is measured then, except the record under `§ Where the deployment configuration lives` of the environment the service reads and what it does without it — from the paragraph opening *The environment variables the table below names* through the one opening *The consequence for the unit* — which is measured on 30 August 2026 because the code it describes landed after this preamble was written. **Two things named below are on disk, and the opening sentence excepts both**: that record, because what the service reads exists while the configuration that would satisfy it still does not, and the two build files `§ Where the deployment configuration lives` names on the far side of the boundary it draws.
...
**What the host must *carry* for the service to start is deliberately not decided here; what the service must be *given* is.** The undecided half is host provisioning — packages, libraries, whatever the executable turns out to need from the machine around it. `§ What this document does not decide` says why, and `deploy/provisioning/` below is the seam it will be written into. The decided half is the service's own configuration surface, which `§ Where the deployment configuration lives` states in full together with what the service does when it is not supplied. **An operator whose service will not start reads that section, not this one**, and the two halves are separated here because that reader has no reason to separate them.
...
**The environment variables the table below names are read at start-up, and they are the whole of the service's configuration surface.** They are new, and this record — this paragraph through the one opening *The consequence for the unit* — is measured on 30 August 2026 rather than at this document's as-of date above: `service/internal/config/owed.go`, called from `service/cmd/turfgps/main.go`, introduced them. Each supplies one enforcement constant that `CalculationSpecification.md § Enforcement constants that do not yet exist` records as owed. **`service/internal/config/owed_corpus_test.go` binds that table to the registry in `service/internal/config/owed.go`, and what it reads here is this section's heading, the table's header row and the table's rows — no prose anywhere in this document, this paragraph included.** So this paragraph describes a binding it is not part of: it would go on asserting one after that test stopped reading the table, with nothing red and nothing to read but the test. That is why what an operator must act on is in the table. A constant added to the registry turns that test red and the table is repaired with it; a count written into a sentence would go quietly wrong instead, in a file nobody debugging a refused start-up would think to check. **How many there are is the table's to say and no sentence's.**

| Variable | Supplies the constant that section names |
|---|---|
| `TURFGPS_UNCERTAIN_STOP_UPPER_BOUND` | The conservative upper bound for an uncertain stop |
| `TURFGPS_IMPLAUSIBLE_GRADIENT_THRESHOLD` | The "implausibly steep" gradient threshold |

**Every constant the table names is owed and unauthored, and no value for any of them may be written here or anywhere under `deploy/`.** None has a value in any document. Which constants are owed, why each is, and which direction each may only be revised in are stated once, in the section cited above; this document records only the deployment-facing half — that a value arrives through the environment, and what the service does when one does not. **What follows from a constant being unset is not stated here for any of them.** It is stated in the section cited above and in the `Risk` field of `docs/Requirements/safety-exclusions.md § FR-091`, which is also the requirement the refusal discharges, and a reader who needs it reads both because neither answers for the other. **What this document does state is narrower, and it is the same for each**: unset, the service does not start. That is the refusal's doing rather than anything a constant gates on its own, and nothing here should be read as attributing a blocking rule to any of them. No constant here is a proposed default awaiting evidence, and none may be given a plausible figure to get a deployment running.

**"Unauthored" and "unset" are not the same state, and only the second is what the service checks.** A constant is unauthored while no document gives it a value — the state every one of them is in, and the state the prohibition above is about. A variable is unset while the environment carries no value for it, which is what the check below tests. So **any non-blank string satisfies that check**: a placeholder gets a host past start-up with the constant still unauthored, and nothing anywhere reports that it did. A service that started is therefore evidence that every variable was set and never evidence that any constant was authored. That gap is what the prohibition is for, and why it is worded against supplying a value rather than against omitting one.

**The check is presence and never shape.** A variable that is unset, or set to a blank value, carries no value. The service then logs every constant it found no value for — all of them, rather than the first — and exits non-zero, so a refused configuration **serves nothing at all** rather than serving requests it cannot plan against. Nothing is parsed: what a value must look like is not settled in any document, and settling it here would be code deciding a specification question.

**What a deployment may rely on is narrower than what the code currently does, and the difference was measured rather than assumed.** `service/cmd/turfgps/main.go` runs the check before it opens its listener, so a refused start binds no port today. **That ordering is not what the tests establish.** `service/cmd/turfgps/startup_refusal_test.go` records, in the part of its opening comment naming what it does not reach, that moving the check below the bind was demonstrated to leave every one of its assertions green; what they establish is that the process never began serving. So `deploy/` may be arranged against a refused service serving nothing, and may not be arranged against the port being free while it refuses — that second property holds now, is not under test, and would go false without anything turning red.

**The consequence for the unit is not a small one — the deployment this document models cannot start the service, and will not be able to until those values are authored.** No value exists for any constant the table names, so no `Environment=` line, and no out-of-repo source reached through the mechanisms above, has anything to supply. The unit in the table above restarts the process on failure, so a host provisioned exactly as this document specifies crash-loops rather than stopping once, and the journal is where the refusal is read. **That is the designed behaviour and not a defect for `deploy/` to work around**: it is the state the service holds until the batch owning each constant authors its value, and a deployment that got the service running by inventing one would have defeated the only thing standing between an unauthored safety constant and a planned journey.
