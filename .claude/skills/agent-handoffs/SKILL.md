...
**The checker is `scripts/gates/output-caps.sh` and it is run as the gate `local-gates § Artifact caps` describes.** Naming it is not decoration: a section whose contract is *a machine reads this* and which names no machine cannot be checked by the person editing it, and the two things this table can be edited into — a row that stops parsing, a cap that stops matching what the instrument enforces — are both invisible from this file.
...
- **`body`** — every character except the artifact's enumerated findings: lines beginning with `|`, and the `findings:` key together with **its block** — every line indented deeper than the key, and every `-` item at the key's own indent. The block ends at the first line that is neither, and that line is counted. **There is no terminating key to find**, which is the rule and not a gap in it: an exclusion that scanned for one ran to end of file when none followed, so a verdict whose findings block is followed by prose measured almost nothing and reported clean. The cap was then opt-out by key ordering, and key ordering is the writer's.
...
**Every row this table holds must parse, and one that does not is named rather than skipped.** A row-shaped line that fails the form above is a defect in the table, not a line the checker was never meant to read — and the failure it produces is silent in both directions at once: the artifact that row governs stops being measured at all, while the coverage the recall corpus prints *improves*, the destroyed row having stopped counting as one that wants a fixture. The checker therefore finds rows by position, from this table's header to the first line that leaves it, and refuses to run at all while any line it finds there fails the form. The two-column `prose_licence` table below is outside that region and is not judged against a contract that was never its own.

**That refusal reaches only the lines the region still holds, and the guarantee that it holds all of them is the corpus's rather than the instrument's.** A row indented out of column zero is not a row-shaped line that failed to parse — it is the line that *leaves* the region, so it and every row beneath it are dropped, and the run reports `clean` at rc 0 with nothing named **for an artifact declaring a row that survived**. An artifact declaring the dropped id is named instead, and the run cannot vouch. Measured on 6 September 2026, `worker_envelope` indented by one space: five rows survive; an artifact declaring `judgment` measures 29 against 6000 and the run reports `clean` at rc 0, while one declaring `worker_envelope` prints `unclassified · no cap-table row for worker_envelope` and exits 2. **What holds this case is `scripts/gates/tests/output-caps-recall.sh`, and it holds part of it.** The corpus counts the rows the region holds against the lines matching the row form *anywhere* in the file, and stops red where the two disagree; a row surviving **beneath** a dropped one still matches that form after leaving the region, which is what makes them disagree. **Re-measured on 17 September 2026, the table having grown from twelve rows to thirteen and every denominator below moving with it — the five above did not, `worker_envelope` sitting sixth at either size:** indenting `reviewer_verdict`, `worker_envelope` or `owner_report` holds `0`, `5` and `10` rows against `12` matching, and each stops the corpus at rc 1. A **last** row leaves nothing beneath it, so both counts fall to `12` together and that check is silent. Indenting `supersession_notice` — the last row, and one a fixture is grounded in — is caught instead by that grounding, at rc 1 with `the cap table has no row for 'supersession_notice'`; moving `owner_report` below it and indenting it there, a last row no fixture is grounded in, leaves the corpus `all passed` at rc 0 reporting `7 of 12`. **A dropped last row that no fixture is grounded in is caught by neither check.** `validation_result` is not that row: indented at twelve of thirteen it holds `11` against `12` and stops the corpus at rc 1, measured the same day.
...
| `validation_result` | `2400` | `body` | `@validation-agent`'s own machine result, posted to the PR when it runs — **not a `reviewer_verdict`** and never carrying that vocabulary; it declares the `sha:` it measured, so a result at a superseded head is distinguishable from one at the tip. **Retrieving one is a discipline rather than a search, and the discipline is `validation-agent § How a validation result is retrieved`** — the selector that finds the class, the closed two-name author allowlist that decides whether what it found may be read at all, and the tie-break that picks one result where several stand. **Read that section before you consume one:** every other row here describes an artifact handed to you, and this is the one a consumer goes and fetches, so the consumer is where the control runs and a consumer that never opened it has run none of it. The gate lines are evidence and are counted; the `findings:` list is the excluded part, and `body` rather than `whole` is what keeps the cap off the defect count — a `whole` cap here would bound how many defects the gates are allowed to find. **The cap is the one result written in this class's own shape, plus a tenth, rounded to the hundred**: `2196`, its author's own figure before five cuts took it to the `1795` posted on 17 September 2026. The two couriered validation reports on #135 measure `3585` and `4253` under this row and are **excluded from that derivation rather than unmeasured** — each writes its findings as prose under `##` headings instead of under a `findings:` key, so `body` excluded nothing from either and both numbers are what an unstructured result costs, and each already stood over the cap its own `orchestrator_comment` row sets, at `2977` and `3517` |
...
**The floor holds in one direction only because the checker refuses the case where it would not.** A fence opens on a run of three or more backticks and closes only on a run **at least as long as the one that opened it**; a shorter run inside an open fence is content, not a delimiter. That length rule is what the toggle needs to be believed: a four-backtick wrapper around a relay carrying its own three-backtick blocks is **two** delimiters, and a state that flipped on all four of those lines balanced, passed the refusal below, and measured the relay as though it were the notice. Fence state is a toggle over the delimiters that rule admits, so an artifact ending inside an open fence has every region after the unmatched one inverted — the relay counted, the prose around it dropped — and that manufactures a false `over` as readily as a false `under`. An over-report is not a floor and no reader is warned by one, so an `own` artifact holding an odd number of fence delimiters, which is the same fact as ending inside an open one, is refused as unmeasurable rather than reported at whichever region the inversion selected.
