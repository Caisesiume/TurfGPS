...
**That refusal reaches only the lines the region still holds, and the guarantee that it holds all of them is the corpus's rather than the instrument's.** A row indented out of column zero is not a row-shaped line that failed to parse — it is the line that *leaves* the region, so it and every row beneath it are dropped, and the run reports `clean` at rc 0 with nothing named. Measured on 6 September 2026: indenting `worker_envelope` by one space leaves five rows, and an artifact declaring one of those five is measured and reported clean. What holds this case is `scripts/gates/tests/output-caps-recall.sh`, which counts the rows this table holds against the rows the checker parsed and stops red on the disagreement — so a row dropped here is caught by the corpus, on the next run, and never by the gate that read it.
