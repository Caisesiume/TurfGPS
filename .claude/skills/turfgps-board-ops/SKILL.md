...
- **The `#37 — see #67` row names `#222` in the table, whose line reads `Blocked by: #41 — see #142`, and the two records carrying the live shape pin it as well.** On live story `#136` every reference is satisfied, so no reading of that line prints a blocker list and only an absence can be asserted of it. Fixture `#206`'s reason names an **open** issue — `#41` — so the over-read prints `#206 (blockers: #41 open)`, and both assertions standing on that fixture red when the boundary is inverted to the whole-line scan. `#222` is asked from `#41`, so the whole list prints, and `#142` is open: declaring the reference behind the dash puts it in the parentheses and reds the row there too.
- **`#223` cannot be made to fail against today's parser, and that is a limit of the fixture rather than evidence it does not have.** The line carries no `#`, so no mutation of the awk can red it. It is there because the row exists and a row carrying no fixture is the defect this table replaces.
