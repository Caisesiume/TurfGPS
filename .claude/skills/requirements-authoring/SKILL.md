...
- **The duty is primary on whoever edits the cited file, and a backstop on `@requirements-librarian`.** A commit that renames a heading, re-splits a section, or retracts what one asserted greps the repository for citations naming it and repairs them **in that same commit** — the only moment at which both the old name and the new one are known, and one that no reader of the citing file ever reaches. The librarian re-resolves the corpus's outbound citations against their cited files' current headings **on every pass**, because the editor may be an agent that never loads this skill. **This file is itself one of the cited files** — the corpus names its sections by heading — so the primary duty falls on the RE family whenever a heading here is renamed. **Neither party discharges the other:** the backstop catches what the primary missed, and catches it only after an interval in which the citation was read as true.
