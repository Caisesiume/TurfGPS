...
- No stop on a **refused road class**, or on any road whose speed limit is not **established** to be at or below the maximum for a stopping road. It is a test the road must pass, so an unknown limit excludes it. **Which classes are refused is not listed here** — the set gained three on 7 August 2026 and this line named two of them until it did, which is what a copy of a moving list is worth on a safety path. Read the classes, what establishes a limit, what does not, and the constant itself from `SPECIFICATION.md § Enforceable exclusions`, and hold no copy here. Note that the speed limb admits a road at exactly the maximum, so it does not catch a fast class posted at that figure; the class limb is what does. A nearby rest area, service road, parking area, or exit may still make the zone accessible; the high-speed carriageway itself never is.
