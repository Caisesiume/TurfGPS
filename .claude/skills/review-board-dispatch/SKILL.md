...
**The table is clone-local durable state**, under `.claude/state/review-claims/`, inside the `/.claude/state/` entry `.gitignore` already carries. **Its scope is one clone and every worktree of that clone, which is one level narrower than the machine.** A linked worktree resolves the same table as the checkout it was made from — that is the property the resolution was built for, and `scripts/loop/claim.sh` argues it where it derives the location — but a *second clone* of the same repository resolves a table of its own. Measured on this host on 6 September 2026: the main checkout and `TurfGPS-wt/144-review-claims` both answered `<repo>/.claude/state/review-claims`, and a fresh clone in a scratch directory answered its own path. That is the right scope for what it coordinates — the judges and reviewers of one working repository, which is where every duplicate dispatch on record happened — but it is not a cross-machine lock, not a cross-clone lock, and it is not published. Two agents told to work in two clones do not share a panel; keep a panel inside one clone. What makes a panel visible to a human is the PR ledger comment, defined in `§ Incremental review validity` below.
...
**The docs row is the one place a deterministic fact stops short of its consequence.** `docs_only` is exact and is a statement about file extensions; whether a docs diff falls inside the assessor's auto-low exemption is a judgement about what it *does*, and that exemption is **stated in its registry row below** and nowhere else — `change-risk-assessor` cites the row rather than restating it, and this table applies the row for the same reason. **PR #67 is the evidence:** `docs_only: true` across three Markdown files, and the authoritative PR-open assessment came back `medium / 0.54` naming `security` as a required lane. The observed behaviour already followed the assessor's row; it was this table that was wrong, and §50 had reserved the semantic half for the assessor and the judge before the case arose. Narrowed 16 August 2026 — `docs/adr/ADR-0002-token-efficiency.md § O17`.
...
|---|---|---|---|---|
| `@change-risk-assessor` | risk classification | Item intake (prediction) and PR open (authoritative, from the diff) | Docs-only **typo, formatting, or link fix** — auto-low, no run. This cell is that exemption's one home; the assessor's own contract cites it rather than restating it | Any force-push or scope change to the PR |
...
| `@linus-security-critic` | security surface | Auth, input validation, spatial queries, stored personal data, plan retrieval, secrets, external requests, network exposure / deployment reachability, supply chain / build provenance, data-touching migrations — **including a document that decides one of them** | Pure styling; docs-only **that decides none of the surfaces this row's `Activate when` names** | Any change to a listed surface |
...
**`docs-only` is a statement about file extensions, not about consequence — and the security row now says so.** A document can decide one of the surfaces that row's `Activate when` already names, and a diff doing so has moved the security surface without changing a line of code. **That column is the whole of the bound** — the amendment reaches what a document decides about a surface listed there, and nothing wider. **PR #67 is the evidence, and is why the row was widened rather than argued:** a judge crossed the old flat `docs-only` exclusion deliberately, recorded why on the PR, and `@linus-security-critic` returned **two high findings on paths to stored personal data**. The exclusion still holds for what it was written for — a typo, a heading, a rewording that settles nothing — and the test is what the document *decides*, not what it is named. Amended 16 August 2026 on that evidence; `docs/adr/ADR-0002-token-efficiency.md § O17` records it.
...
| **high** | the medium set + the architecture lane + the security lane where flagged + `@confidence-assessor` **always**. **Every mandatory verdict-producing reviewer must return `pass`** |

Plus, at every tier without exception: `@safety-sentinel` on any safety-path diff.

**High's merge condition reaches the verdict-producing lanes and no others**, which is what the qualifier above buys, and *verdict-producing* is used here in exactly the sense `§ The claim table covers verdict-producing reviewer lanes only` below fixes it — the two bullets that claim, `@validation-agent` among them in its own `pass`/`fail` vocabulary. The unqualified form was unsatisfiable by its own letter: that roster's third bullet holds lanes returning no ruling word at all, and this row makes one of them — `@confidence-assessor` — mandatory always, so a condition demanding `pass` from *every* mandatory reviewer demanded a word one of its own mandatory lanes never emits. **The qualifier does not soften the tier and does not release `@validation-agent`**, which is inside the term and whose `pass` this condition still requires. A non-producing lane is still mandatory and still convened; `@confidence-assessor` is acted on at `@pr-judge § Phase 6`. What the qualifier states is only which lanes the `pass` test can be applied to.
