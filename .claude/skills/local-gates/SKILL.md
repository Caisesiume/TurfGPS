...
**The working directory is now the Makefile's job, and that is the single thing it exists to get right.** Run from the repository root, every Go command resolves against nothing — `Architecture.md § D8` leaves no module there — so all five find no Go files rather than no faults, exit zero, and print exactly what a clean module prints. The old defence was an agent remembering `cd service` on every command line, and a shell that resets between commands is back at the root by the second one. `§ When these activate` below requires every recipe to carry its own directory, which replaces that memory with a property of the file — reaching the false pass now takes an edit rather than a lapse.
...
npm resolves a script against the nearest `package.json`, and there is none at the repository root, so from there these three do not run the client's build, lint, or tests at all. That failure is at least loud — npm reports a missing script, where a bare Go command at the root exits zero having read nothing. This block is wrong from the root and says so, and `cd web` is the whole of its defence until a target covers this stack.
