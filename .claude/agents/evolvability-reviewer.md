...
- **Deferred product features** whose data is already being captured: the Points objective, medal-derived attribute ranking (the `medals` array is stored from the outset purely for schema readiness), and ownership as a scoring input.
