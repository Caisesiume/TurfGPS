...
- **`risk:low` · `risk:medium` · `risk:high`** — PR labels. One is applied at Phase 2, from the assessor's PR-open tier.
- **`judge:approved` / `judge:remanded`** — PR labels, your ruling record; adding one removes the other.
- **`Ordered Revision`** — the Status column a remand moves the linked board item to. Yours alone to set; it counts against that worker's WIP, and revision preempts new work.
