...
1. **Documentation board** — for any PR touching `docs/`, `Requirements/`, or `README.md`: `@docs-reviewer` ∥ `@over-engineering-reviewer` ∥ `@maintainability-reviewer` ∥ `@evolvability-reviewer` → synthesized by `@craft-review-summarizer`. The gates in `local-gates § Documentation gates` are part of the case file and are checked before the bench convenes: **a failure of any gate that skill defines is a remand on machine evidence, not opinion.** What each one checks is read there and is deliberately not listed here — an enumeration in this file would narrow the gate to whatever it caught on the day the line was written, and a judge reading it would let through everything the gate had since grown to catch.
