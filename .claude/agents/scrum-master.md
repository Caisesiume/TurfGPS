...
### Phase 3 — Readiness
For each Backlog candidate, in order: **traceability** (label, Milestone, `Resolves:` — a `Task` is exempt per `turfgps-board-ops § Labels`; an untraceable story routes to @requirements-engineer and does not promote) · **the persisted hard edges** in its `## Dependencies` section, read per `turfgps-board-ops § The dependency representation`, where `Soft dependency:` lines are not gates and never hold an item · **every hard blocker successfully complete** — for an issue that is what `ADR-0003 § A3` ratifies and `scripts/loop/dependents.sh` implements, and **it admits more closed states than a close marked *completed***, so take the verdict from there rather than from this line; a blocker that is itself a pull request and is *merged* is complete too, while a blocker whose work merged on `main` under a **still-open issue** reads `OPEN` and still blocks — the script cannot see that the work landed, so closing the issue is the human step that makes the edge satisfied, never a promotion you make on the merge alone; a prerequisite closed as *not planned* or as a duplicate, and a pull request closed without merging, never did the work, still block, and are a `dependency_finding` rather than a promotion (`turfgps-board-ops § Satisfied is not removed`) · **no explicit blocking state left** (`awaiting-human`, an open remand, a stated hold, or a `## Dependencies` section still reading `_Pending @backlog-dependency-planner._` — unplanned is not unblocked, per `turfgps-board-ops § The dependency representation`). Where a merge woke this run, `scripts/loop/dependents.sh <merged-issue>` has already done the third step: its `eligible:` list is your candidate set, its `still_blocked:` list is your evidence for holding the rest, and its `not_completed:` line names the prerequisites that are closed but never completed.
