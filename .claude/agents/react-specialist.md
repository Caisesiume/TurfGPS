...
- **Must NOT run when:** No item is assigned; the item has no `web` surface; the frontend stack is dormant, which is derived from the tree per `codebase-map § Which map is authoritative — check the tree, do not assume` and never from this line.
