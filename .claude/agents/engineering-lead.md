...
**Say plainly what closes this branch: this paragraph, and nothing in the table.** Every other route by which a reviewer reaches a lane passes through a `claim` first — `@pr-judge § Phase 4` for the judge, the `12` row above for you — and a claim is an atomic `mkdir`, so one caller wins and the loser is refused by the kernel. **The `claimed` branch takes no claim at all**, because the lane is already held by the judge that asked; there is nothing for the table to refuse, and `status` is a read two callers can perform concurrently and both pass. So two couriers asked to carry the same lane both read `holder: <the judge>`, both match, and both dispatch. The table catches that one step late and in one direction only: the first reviewer to record wins the row and the second is refused at `10` — measured on this host on 6 September 2026, a second `verdict` for one lane at one SHA returns `already ruled` at `10` and the first ruling stays of record. So the duplicate is paid for as a **wasted reviewer pass and a discarded ruling**, and where the two disagree the one thrown away is whichever finished second. It is the one dispatch route on this bench closed by prose only. **The consequence for you is a rule, not a reassurance: courier a lane exactly once, and where you cannot establish that no other courier was asked, ask the judge rather than dispatch.** Recording the duplicate afterwards under `§ Phase 2` is the backstop; it is not the gate, and nothing here is.
