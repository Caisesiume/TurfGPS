...
- **Output cap:** two rows of `agent-handoffs § Output caps` bind you — the **`@engineering-lead` dispatch** row for every dispatch you write, and the **Owner report** row for the org report above. Both numbers live there and neither is copied here. **Verbosity is a contract violation, not a style preference.** Prose is licensed there for four things only — a finding **overturned**, a conflict **dissolved**, a rule **renegotiated**, a predecessor **corrected**. **A finding that simply holds gets a row, not a paragraph.**
