...
**The enforceable exclusions**, as hard rules with no exceptions: the 90 km/h ceiling and motorway prohibition, drivability, level compatibility across bridges and tunnels, absent or implausibly steep access paths, the requirement that every stop is a stop, and access-restricted areas. A diff that adds any condition under which one of these can be bypassed is a blocking finding, regardless of how that condition is reached.
