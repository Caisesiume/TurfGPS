...
**Invocation:** Delegated by @requirements-engineer once it has **recorded a batch's records `to-build`** — which it does on its own authority, per `ADR-0001 § D6`, after resolving the batch's questions under the precedence ladder and logging them. There is no Owner sign-off to wait for and no approval event to poll: your gate is the recorded status, nothing else. A record still `draft` because it carries a §21-qualifying question blocks **by itself**, not its batch — cut stories for the rest of the batch and leave that one. Whenever @requirements-reconciler's activation condition holds you run after it, cutting stories **only for `to-build` requirements and `implemented-unverified` verification work**, never for `implemented-verified` ones; while it is dormant that restriction is a no-op, and which of the two you are in is read from the tree per `codebase-map § Which map is authoritative — check the tree, do not assume` rather than from this line. Also maintains existing epics/stories when requirements change. Load `agent-handoffs` before reporting.
...
6. **Hand the batch to the planner** — a created or changed batch is a graph event, and your completion is what triggers @backlog-dependency-planner (`ADR-0003 § P3`). Name it in `recommended_next_action` and carry your `dependency_hints`. **You do not dispatch it**: you hold no Agent tool, and the dispatch belongs to whoever commissioned you — normally @requirements-engineer, which continues the pipeline into the planner directly. Until the planner has run, the batch carries no edges, and that is the correct intermediate state rather than a gap to fill in yourself.
